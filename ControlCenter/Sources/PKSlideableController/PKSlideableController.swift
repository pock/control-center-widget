...
class PKSlideableController: PKTouchBarController {
    ...
    /// Core
    private weak var down_item: ControlCenterItem? { didSet { leftItemView.image  = down_item?.icon } }
    private weak var up_item:   ControlCenterItem? { didSet { rightItemView.image = up_item?.icon   } }
    ...
    deinit {
        print("[PockSlideableController]: Deinit slideable controller")
    }
